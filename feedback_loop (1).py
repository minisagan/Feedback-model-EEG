from extras import get_X, get_Y_hat, get_sq_err_t, get_total_sq_err, avg_batch_error, err_derivative_k, err_derivative_h
import numpy as np 
...
class Feedback: 
    ...
    def fit(self, x, y, k, h, lambda_k = 1e-7, lambda_h = 1e-7, batchsize=500):
        '''
    ...
    def predict (self, x, y, M = 35, N = 35):
        Y_hat = np.zeros(M)
        size = int(x.size)
