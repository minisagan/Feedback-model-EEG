...
def get_sq_err_t(x, y, k, h, s, t): 
    Y_hat = get_Y_hat(x, s, t) #note that this gives t-1 since ranges are not inclusive
    X = get_X(x, t, N)
    y_hat = k @ X + h @ Y_hat
...
def get_total_sq_err(x, y, k, h, start, stop):
    ...
    for t in range(start, stop+1): 
        squared_error = get_sq_err_t(x, y, k, h, start, t)
...
def err_derivative_k(x, y, t, k=k, h=h, start = 0):
...
def err_derivative_h(x, y, t, k=k, h=h, start = 0): 
